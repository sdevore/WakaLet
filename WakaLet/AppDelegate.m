...
- (void)checkAPIAndBuildMenu {
    ...
    if (api_key == nil || [api_key isEqualToString:@""]) {
        [self addItemInMenuIfNeeded:
                  [[NSMenuItem alloc] initWithTitle:@"Set API key" action:@selector(api) keyEquivalent:@"a"]];
    }
    ...
    // If there is one, initiate the request
        ...
        NSString *someDaysAgoDate =
            [formatter stringFromDate:[[NSDate date] dateByAddingTimeInterval:3600 * 24 * 7 * -1]];

        // Build the parameters for the string
        NSString *dataString =
            [NSString stringWithFormat:@"?start=%@&end=%@&api_key=%@", someDaysAgoDate, todayDate, api_key];

        // The general API url
        NSString *urlString =
            [@"https://wakatime.com/api/v1/users/current/summaries" stringByAppendingString:dataString];
        ...
        [[NSUserDefaults standardUserDefaults] setValue:[NSDate date] forKey:@"refresh"];
            ...
            sendAsynchronousRequest:request
                  ...
                  completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                      ...
                      if (connectionError != nil) {
                          [self addItemInMenuIfNeeded:[[NSMenuItem alloc]
                                                          initWithTitle:connectionError.localizedDescription
                                                                 action:@selector(api)
                                                          keyEquivalent:@""]];
                      }
                      ...
                      // Else just try to build the menu
                          ...
                          if (err == nil) {
                              [self performSelectorOnMainThread:@selector(buildMenuWithData:)
                                                     withObject:dict
                                                  waitUntilDone:NO];
                          }
...
- (NSString *)input:(NSString *)prompt defaultValue:(NSString *)defaultValue {
    NSAlert *alert =
        [NSAlert alertWithMessageText:prompt
                        defaultButton:@"OK"
...
- (void)buildMenuWithData:(NSDictionary *)dict {
    ...
    for (int i = 0; i < [dataObjArray count]; i++) {
        ...
        NSMenuItem *dateItem =
            [[NSMenuItem alloc] initWithTitle:date_human.capitalizedString action:@selector(open:) keyEquivalent:@""];
    ...
    NSMenuItem *timeSinceRefreshItem =
        [[NSMenuItem alloc] initWithTitle:[self getTimeSinceRefreshTitle] action:nil keyEquivalent:@""];
    ...
    [self addItemInMenuIfNeeded:[[NSMenuItem alloc] initWithTitle:@"Refresh now"
                                                           action:@selector(checkAPIAndBuildMenu)
                                                    keyEquivalent:@"r"]];
    [self addItemInMenuIfNeeded:
              [[NSMenuItem alloc] initWithTitle:@"Dashboard" action:@selector(dashboard) keyEquivalent:@"d"]];
    [self addItemInMenuIfNeeded:
              [[NSMenuItem alloc] initWithTitle:@"Set API key" action:@selector(api) keyEquivalent:@"a"]];
}
